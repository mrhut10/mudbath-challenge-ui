...
function getJSONFileData<T, K extends keyof T>(
  path: string,
  name: string,
  keyName: K,
) {
  ...
  const [stateValue, setSateValue] = useState(initialState) as [
    JSONFileState<T, K>,
    (a: JSONFileState<T, K>) => any,
  ]

  const itemByKeyValue = (keyValue: T[K]): T =>
    stateValue.data.find((item) => item[keyName] === keyValue)
  ...
  const updateItemByKey = (keyValue: T[K], newItem: T) => {
    const oldItemIndex = stateValue.data.findIndex(
    ...
    if (
      ...
      throw new Error(
        "can't change as will create conflict of multiple items with same id",
      )
    }

    return setSateValue({
      ...stateValue,
      data: stateValue.data.map((item, index) =>
        index === oldItemIndex ? newItem : item,
      ),
    })
  }
  ...
  return {
    stateValue,
    selectItemByKey,
    danerouslySetAllValues: setSateValue,
  }
}
