    ...
    case PRODUCT_ADD:
      {
        ...
        if (
          ...
          a.payload.every((item) => isProduct(state, item))
        ) {
          return {
            ...state,
            allProducts: [
              ...state.allProducts,
              ...(a.payload as productInterface[]),
            ],
          }
        } else if (a.payload && isProduct(state, a.payload)) {
          return {
            ...state,
            allProducts: [...state.allProducts, a.payload as productInterface],
          }
        }
      }
      break
    case PRODUCT_EDIT:
      {
        ...
        if (
          ...
          (a.payload.id === a.payload.product.id ||
            state.allProducts.every((item) => item.id !== a.payload.product.id))
        ) {
          return {
            ...state,
            allProducts: state.allProducts.map((item) =>
              item.id === a.payload.id ? a.payload.product : item,
            ),
            productDialogState: state.productDialogState
              .map((item) =>
                item.id === a.payload.id
                  ? { ...item, id: a.payload.product.id }
                  : item,
              )
    ...
    case PRODUCT_DIALOG_VIEW:
      {
        const a:
          | ACTION_PRODUCT_DIALOG_EDIT
          | ACTION_PRODUCT_DIALOG_VIEW = action
        return {
          ...state,
          productDialogState: [...state.productDialogState, a.payload],
        }
      }
      break
...
function isProduct(state, item) {
  ...
  if (!item.price?.base || typeof item.price.base !== 'string') {
    return false
  }
  ...
  if (
    ...
    !item.relatedProducts.every(
      (relID) => typeof relID === 'number' && Number.isInteger(relID),
    )
  ) {
    return false
  }
