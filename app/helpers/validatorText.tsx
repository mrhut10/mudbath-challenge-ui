...
class ValidatorText extends Validator {
  ...
  required(failMessage: string = 'Value required'): ValidatorText {
    this.rules.push(
      Validator.resultGenerator(
        failMessage,
        (text: string) => typeof text === 'string' && text.length > 0,
      ),
    )
    return this
  }
  minLength(
    failMessage: string = 'Value to short',
    minLen: number,
  ): ValidatorText {
    this.rules.push(
      Validator.resultGenerator(
        failMessage,
        (text: string) => typeof text === 'string' && text.length >= minLen,
      ),
    )
    return this
  }
  maxLength(
    failMessage: string = 'Value to long',
    maxLen: number,
  ): ValidatorText {
    this.rules.push(
      Validator.resultGenerator(
        failMessage,
        (text: string) => typeof text === 'string' && text.length <= maxLen,
      ),
    )
    return this
  }
  isNumber(failMessage: string): ValidatorText {
    this.rules.push(
      Validator.resultGenerator(
        failMessage,
        (text: string) => typeof text === 'string' && regexNumber.test(text),
      ),
    )
    return this
  }
  isInteger(failMessage: string): ValidatorText {
    this.rules.push(
      Validator.resultGenerator(
        failMessage,
        (text: string) => typeof text === 'string' && regexInteger.test(text),
      ),
    )
    return this
  }
  ...
  isUnquie(failMessage: string, takenValues: string[]): ValidatorText {
    this.rules.push(
      Validator.resultGenerator(
        failMessage,
        (text) => !takenValues.includes(text),
      ),
    )
    return this
  }
