...
class dataSource {
  ...
  getEntries(sourceName: string, userName: string): databaseReply {
    ...
    const permitted =
      source &&
      source.rights.find(
        (right) => right.userName === userName && right.canView,
      )
    return {
      status: permitted ? 'success' : 'fail',
      message: permitted
        ? 'entries attached'
        : 'either no source or no permission',
  ...
  EditEntry(
    ...
    userName: string,
    index: number,
    fields: [{ name: string | number | symbol; value: any }],
  ): databaseReply {
    const source = this.#sources.find((source) => source.name === sourceName)
    const fullPermision =
      source &&
      fields.every((field) =>
        source.rights.find(
          (right) =>
            ...
            right.fieldEditRights.includes(field.name),
        ),
      )
  ...
  AddEntry(sourceName: string, userName: string, entry: object): databaseReply {
    ...
    const permitted =
      source &&
      source.rights.find(
        (right) => right.userName === userName && right.canAddEntry,
      )

    if (!permitted) {
      return {
        status: 'fail',
        message: 'source or permission not found',
        data: undefined,
      }
    }
  ...
  DeleteEntry(
    ...
    const permitted =
      source &&
      source.rights.find(
        (right) => right.userName === userName && right.canDeleteEntry,
      )

    if (!permitted) {
      return {
        status: 'fail',
        message: 'source or permision not found',
        data: undefined,
      }
    }
    ...
    return {
      status: 'success',
      message: `entry at index ${index} was removed`,
      data: undefined,
    }
  }
  AddRights(
    sourceName: string,
    userName: string,
    right: datasourceRights,
  ): databaseReply {
    const source = this.#sources.find((source) => source.name === sourceName)
    const permitted =
      source && source.rights.find((right) => right.canAddPermission)
